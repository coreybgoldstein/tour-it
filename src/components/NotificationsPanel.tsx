...
export interface NotificationRow {
  ...
  // Course logo for any notification with a /courses/{id} linkUrl —
  // rendered as a small crest on the corner of the icon/thumbnail so
  // the user can see at a glance which course the notification is about.
...
export default function NotificationsPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Build the set of course IDs referenced by notification linkUrls
      // so we can fetch all their logos in one query. clip_tag links
      // resolve later from the Upload row, so we union both sources.
      ...
      // Keyed by Upload.id — clip_tag rows resolve their course via the
      // Upload they reference, not via linkUrl, so we track separately.
      const clipCourseIdMap: Record<string, string> = {};
      ...
      if (courseIdsFromLinks.size > 0) {
        ...
        (courseRows || []).forEach((c: { id: string; logoUrl: string | null }) => {
      ...
      setNotifications(notifs.map(n => {
        ...
        // Course id comes from the Upload row for clip_tag (the link
        // may not exist yet) and from the linkUrl for everything else.
                      ...
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        {n.type === "clip_tag" && n.clipThumbnail ? (
                          <>
                            ...
                            {n.courseLogoUrl && (
                              <img src={n.courseLogoUrl} alt="" style={{ position: "absolute", bottom: -5, right: -5, width: 28, height: 28, borderRadius: 0, border: "2px solid #07100a", background: "#fff", objectFit: "cover", boxShadow: "0 1px 4px rgba(0,0,0,0.5)", pointerEvents: "none" }} />
                            )}
                          </>
                        ) : (
