...
type FeedClip = {
  ...
  createdAt: string;
  // Original uploader's user id + username, set only when ownership has
  // been transferred via an approved hero tag. Renders the
  // "uploaded by @x" attribution chip beneath the username row on the
  // clip overlay, and the chip is tappable into the uploader's profile.
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 10px 12px" }}>
        {featured && (
          <div style={{ marginBottom: 4, maxWidth: "100%" }}>
            <span style={{ display: "block", fontFamily: "'Outfit', sans-serif", fontSize: 8, fontWeight: 800, color: GOLD, letterSpacing: "0.16em", textTransform: "uppercase", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", textShadow: "0 1px 2px rgba(0,0,0,0.7)" }}>
              [ {featured.pillText} ]
            </span>
          </div>
        )}
        ...
        {course.uploadCount > 0 && !featured && (
          <div style={{ marginTop: 5 }}>
            <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#4da862", textShadow: "0 1px 2px rgba(0,0,0,0.6)" }}>{course.uploadCount} Clips</span>
          </div>
        )}
      </div>
    </div>
...
function RightPanel({ userId, avatarUrl, username, rank, courseId, courseName, clipId, holeNumber, liked, onLike, likeCount, onLikeCountTap, onComment, commentCount, commented, onTapUser, onIntel, intelOpen, onReport, onEdit, isFollowing, onFollow, hasAttribution }: {
  ...
  // Tap the count under the heart to open the "Who liked this" sheet.
  // Optional so callers without that wiring just don't get the
  // affordance — the heart still toggles likes either way.
  onLikeCountTap?: () => void;
  ...
  isFollowing?: boolean;
  onFollow?: () => void;
  // When true, the username overlay grew a second row (attribution chip).
  // Shift the right rail's bottom up so the bottommost button clears the
  // new taller overlay block on the left.
  ...
  return (
    <div style={{ position: "absolute", right: 12, bottom: "calc(150px + env(safe-area-inset-bottom))", display: "flex", flexDirection: "column", alignItems: "center", gap: 14, zIndex: 10 }}>
      ...
      {/* Like — heart toggles the user's like; the count below it is
          its own tap target that opens the "Who liked this" sheet
          (Instagram-style). Two affordances in one stacked column. */}
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
        <button onClick={onLike} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}>
        ...
        <button
          onClick={onLikeCountTap}
          disabled={!onLikeCountTap || likeCount === 0}
          style={{ background: "none", border: "none", padding: "0 4px", cursor: onLikeCountTap && likeCount > 0 ? "pointer" : "default" }}
        >
...
const SeriesCard = memo(function SeriesCardImpl({
  item, isActive, onTapCourse, onTapUser, onComment, onShowLikes, currentUserId, followingIds, onFollow, likedIds, commentedIds, commentOpen,
}: {
  item: Extract<FeedItem, { type: "series" }>;
  isActive: boolean;
  onTapCourse: () => void; onTapUser: () => void; onComment: () => void;
  // Open the "Who liked this" sheet for the currently-active shot.
  ...
  return (
    ...
    >
      ...
      <RightPanel userId={item.userId} avatarUrl={item.avatarUrl} username={item.username} rank={item.rank} courseId={item.courseId} courseName={item.courseName} clipId={activeShot?.id} holeNumber={item.holeNumber ?? null} liked={seriesLiked} onLike={handleSeriesLike} likeCount={seriesLikeCount} onLikeCountTap={onShowLikes && activeShot?.id ? () => onShowLikes(activeShot.id) : undefined} onComment={() => { if (intelOpen) setIntelOpen(false); onComment(); }} commentCount={item.shots[0]?.commentCount || 0} commented={!!(activeShot?.id && commentedIds?.has(activeShot.id))} onTapUser={onTapUser} onIntel={hasNotes ? () => setIntelOpen(o => !o) : null} intelOpen={intelOpen} isFollowing={followingIds?.has(item.userId)} onFollow={currentUserId && currentUserId !== item.userId ? () => onFollow?.(item.userId) : undefined} hasAttribution={!!activeShot?.uploadedByUsername} />
      ...
      {(item.username || formatClipDate(activeShot?.datePlayedAt, activeShot?.createdAt)) && (
        <div style={{ position: "absolute", left: 16, right: 80,
          ...
          {activeShot?.uploadedByUsername && (
            <button
              onClick={(e) => { e.stopPropagation(); router.push(`/profile/${activeShot.uploadedByUserId ?? ""}`); }}
              ...
              <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 500, color: "rgba(255,255,255,0.75)" }}>
                uploaded by @{activeShot.uploadedByUsername}
              </span>
            </button>
          )}
        </div>
...
const VideoCard = memo(function VideoCardImpl({
  clip, isActive, onTapCourse, onTapUser, onComment, onShowLikes, onEnded, onReport, onEdit, currentUserId, followingIds, onFollow, likedIds, commentedIds, commentOpen,
}: {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100svh", ...(isDesktop ? { background: "#000", display: "flex", justifyContent: "center" } : {}) }}>
      ...
      <RightPanel userId={clip.userId} avatarUrl={clip.avatarUrl} username={clip.username} rank={clip.rank} courseId={clip.courseId} courseName={clip.courseName} clipId={clip.id} holeNumber={clip.holeNumber ?? null} liked={liked} onLike={handleLike} likeCount={likeCount} onLikeCountTap={onShowLikes ? () => onShowLikes(clip.id) : undefined} onComment={() => { if (intelOpen) setIntelOpen(false); onComment(); }} commentCount={clip.commentCount} commented={!!commentedIds?.has(clip.id)} onTapUser={onTapUser} onIntel={hasNotes ? () => setIntelOpen(o => !o) : null} intelOpen={intelOpen} onReport={onReport} onEdit={onEdit} isFollowing={followingIds?.has(clip.userId)} onFollow={currentUserId && currentUserId !== clip.userId ? () => onFollow?.(clip.userId) : undefined} hasAttribution={!!clip.uploadedByUsername} />
      ...
      {(clip.username || formatClipDate(clip.datePlayedAt, clip.createdAt)) && (
        ...
        // clips just clear the BottomNav on Face ID phones. When the
        // attribution chip is present, the whole stack shifts up by
        // 30px (video) / 30px (photo) so the chip slots in BELOW the
...
export default function HomeClassicEditorial() {
  const router = useRouter();
  const isDesktop = useIsDesktop();
  ...
  // "Who liked this" sheet — tap the count under the heart to open.
  // Holds the uploadId so the LikesSheet can fetch the right Like rows.
  ...
  useEffect(() => {
    ...
    async function loadFeed() {
      ...
      const SELECT = "id, mediaUrl, cloudflareVideoId, mediaType, courseId, holeId, strategyNote, clubUsed, windCondition, shotType, likeCount, commentCount, userId, uploadedByUserId, seriesId, seriesOrder, yardageOverlay, datePlayedAt, createdAt";
      ...
      const uploaderIds = [...new Set(finalUploads.map((u: any) => u.uploadedByUserId).filter(Boolean) as string[])];
      ...
      const enriched: FeedClip[] = finalUploads.map((u: any) => {
        ...
        // Attribution chip — only render when ownership was transferred
        // AND the original uploader is distinct from the current owner.
    ...
    <main style={{ height: "100svh", background: "#07100a", overflow: "hidden", position: "relative" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Editorial: one continuous dark environment — no green safe-area
           stripe, no speckle. Grain lives in .editorial-grain instead. */
        html, body { background: #07100a; }
        body { overflow: hidden; }
        /* Subtle film grain, scoped to the discovery screen (absolute,
           contained) so it never overlays the video feed below. */
        .editorial-grain {
          position: absolute; inset: 0; pointer-events: none; z-index: 5;
          opacity: 0.05; mix-blend-mode: overlay; background-size: 180px 180px;
      ...
      {menuOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200 }}>
          ...
          <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: "72vw", maxWidth: 300, background: "#07100a", borderRight: "1px solid rgba(77,168,98,0.18)", display: "flex", flexDirection: "column", paddingTop: 64, paddingBottom: 40 }}>
            ...
            {/* Logo header — Invite moved out of here to a dedicated
                spot at the bottom of the menu (beta feedback: invite
                deserves more room + clearer placement, not a small
                pill at the top). */}
            <div style={{ paddingLeft: 24, paddingRight: 16, paddingBottom: 28, borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center" }}>
              <img src="/tour-it-logo-full.png" alt="Tour It" style={{ height: 36, width: "auto" }} />
            </div>

            {/* Nav items — kept in sync with TourItTopBar's menu so the home
                hamburger has the same options + icons as every other page.
                If you add an item to one, add it to the other.
                Order: top = thing the user is most likely to want
                (Tee Up = plan a round / game / trip), then engagement
                surfaces (Play Tour It, Leaderboard, Notifications,
                Feedback), then About + legal at the bottom. About used
                to be #1 — moved down per beta feedback ("the first
                thing should be the thing you're driving the most"). */}
            <div style={{ flex: 1, display: "flex", flexDirection: "column", paddingTop: 12 }}>
              {[
                ...
                {
                  label: "Play Tour It",
                  // Trophy silhouette — the gamification hub.
            ...
            {/* Invite — own dedicated row near the bottom so it gets
                the real estate it deserves. Beta feedback: the tiny
                top-row pill was easy to miss. */}
            {user && (
              <button
                onClick={() => { setMenuOpen(false); router.push("/invite"); }}
                style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 24px", background: "rgba(77,168,98,0.08)", border: "none", borderTop: "1px solid rgba(77,168,98,0.2)", cursor: "pointer", color: "#4da862", fontFamily: "'Outfit', sans-serif", fontSize: 15, fontWeight: 600, width: "100%", textAlign: "left" }}
              >
      ...
      >
        ...
        <div className="feed-item" style={
          ...
          {/* Header — flattened into the continuous dark environment
              (no green gradient stripe, no speckle). */}
          <div style={{ position: "relative", background: "#07100a", borderBottom: "1px solid rgba(255,255,255,0.07)", flexShrink: 0 }}>
          ...
          <div style={{ padding: "0 20px 20px", flexShrink: 0 }}>
            <button
              ...
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 11, background: "transparent", border: "1px solid rgba(255,255,255,0.18)", borderRadius: 0, padding: "16px 18px", cursor: "pointer" }}
            >
              ...
              <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 15, color: "rgba(255,255,255,0.55)" }}>Find a course — name, city, or state</span>
            </button>
          </div>
          ...
          {user === null && (
            <div style={{ padding: "0 20px 18px", flexShrink: 0 }}>
              <button
                ...
                style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", background: "transparent", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 0, padding: "12px 16px", cursor: "pointer" }}
              >
                ...
                <div style={{ background: "#4da862", borderRadius: 0, padding: "8px 16px", flexShrink: 0, marginLeft: 12 }}>
                  <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#07100a" }}>Join</span>
                </div>
              </button>
            </div>
          )}
              ...
              <div style={{ flexShrink: 0 }}>
                ...
                <div className="courses-row">
                  {deduped.length > 0 ? deduped.map(course => (
                    ...
                    <div key={i} className="course-card course-card-compact" style={{ borderRadius: 0, flexShrink: 0, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }} />
                  ))}
                </div>
              </div>
            );
          ...
          {locationStatus !== "denied" && (
            <div className="discovery-section" style={{ flexShrink: 0, marginTop: 10 }}>
              <div style={{ padding: "0 20px 10px", display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <button onClick={() => router.push("/map")} style={{ display: "flex", alignItems: "center", gap: 4, background: "transparent", border: "1px solid rgba(77,168,98,0.5)", borderRadius: 0, padding: "3px 9px", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                  ...
                  <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#4da862" }}>Map</span>
                </button>
                <div style={{ flex: 1 }} />
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "nowrap", flexShrink: 0 }}>
                  {locationStatus === "granted" && [10, 25, 50].map(r => (
                    <button key={r} onClick={() => { setNearMeRadius(r); fetchNearMe(r); }} style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", color: nearMeRadius === r ? "#07100a" : "rgba(255,255,255,0.45)", background: nearMeRadius === r ? "#4da862" : "transparent", border: `1px solid ${nearMeRadius === r ? "#4da862" : "rgba(255,255,255,0.14)"}`, borderRadius: 0, padding: "3px 9px", cursor: "pointer", whiteSpace: "nowrap" }}>
                      {r}MI
                    </button>
                  ))}
                  {locationStatus === "idle" && (
                    <button onClick={() => fetchNearMe()} style={{ display: "flex", alignItems: "center", gap: 5, background: "transparent", border: "1px solid rgba(77,168,98,0.45)", borderRadius: 0, padding: "4px 12px", cursor: "pointer" }}>
                      ...
                      <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#1a9e42" }}>Enable</span>
                    </button>
                  )}
                </div>
              </div>
          ...
          {/* Editorial film grain — absolute + contained to this discovery
              screen so it never overlays the video feed below. */}
        ...
        {!loading && feedItems.length === 0 && (
          <div className="feed-item" style={{ height: "100svh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, padding: 32, textAlign: "center" }}>
            ...
            {user && <button onClick={() => router.push("/upload")} style={{ background: "#4da862", border: "none", borderRadius: 0, padding: "13px 32px", color: "#07100a", fontSize: 13, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: "'Outfit', sans-serif" }}>Upload a clip</button>}
            {!user && <a href="/signup" style={{ background: "#4da862", border: "none", borderRadius: 0, padding: "13px 32px", color: "#07100a", fontSize: 13, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", textDecoration: "none", fontFamily: "'Outfit', sans-serif" }}>Join Tour It</a>}
          </div>
        )}
      </div>
      ...
      {showPushPrompt && user && (
        ...
        >
          ...
          <button
            ...
            disabled={pushBusy}
            style={{ background: "#4da862", border: "none", borderRadius: 0, padding: "8px 16px", fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#07100a", cursor: pushBusy ? "default" : "pointer", flexShrink: 0 }}
          >
            {pushBusy ? "…" : "Turn on"}
          </button>
          <button
