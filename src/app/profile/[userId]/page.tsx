...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    createClient().from("Comment").select("id, body, createdAt, userId, User:userId(username, avatarUrl, UserProgression(rank))").eq("uploadId", commentUploadId).order("createdAt", { ascending: true })
      .then(({ data }) => {
        if (data) setCommentItems(data.map((c: any) => ({ id: c.id, body: c.body, createdAt: c.createdAt, userId: c.userId, username: c.User?.username || "golfer", avatarUrl: c.User?.avatarUrl || null, rank: (c.User?.UserProgression as any[])?.[0]?.rank || null })));
  ...
  async function deleteComment(c: { id: string; userId: string }) {
    if (!currentUserId || c.userId !== currentUserId || !commentUploadId) return;
    const supabase = createClient();
    ...
    setTaggedUploads(prev => prev.map(u => u.id === commentUploadId ? { ...u, commentCount: Math.max(0, (u.commentCount || 0) - 1) } : u));
    if (!remaining.some(ci => ci.userId === currentUserId)) {
      setCommentedIds(prev => { const next = new Set(prev ?? []); next.delete(commentUploadId); return next; });
    }
  }

  async function saveCommentEdit() {
    if (!currentUserId || !editingCommentId || !editingCommentText.trim()) return;
    const body = editingCommentText.trim();
    const supabase = createClient();
    const { error } = await supabase.from("Comment").update({ body, updatedAt: new Date().toISOString() }).eq("id", editingCommentId).eq("userId", currentUserId);
    if (error) return;
  ...
  return (
    <main style={{ background: "#07100a", minHeight: "100dvh", fontFamily: "'Outfit', sans-serif", color: "#fff", paddingBottom: isDesktop ? 0 : "calc(104px + env(safe-area-inset-bottom))", paddingLeft: isDesktop ? 72 : 0, maxWidth: isDesktop ? 760 : undefined }}>
      ...
      {/* Hidden while the full-screen clip feed is open so clip pages
          have no bottom nav bar. */}
      {!feedOpen && <BottomNav />}
