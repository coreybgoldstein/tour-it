...
export default function TripPage() {
  ...
  async function uploadScorecardPhoto(file: File) {
    setScorecardUploading(true);
    try {
      const supabase = createClient();
      const ext = (file.name.split(".").pop() || "jpg").toLowerCase();
      const path = `scorecards/trip-${id}-${Date.now()}.${ext}`;
      ...
      const res = await fetch(`/api/trips/${id}/scorecard-photo`, {
        method: "POST",
      ...
      setTrip(prev => prev ? { ...prev, scorecardPhotos: photos } : prev);
  ...
  async function removeScorecardPhoto(url: string) {
    try {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();
      const token = session?.access_token;
  ...
  const flavorEyebrow = flavor === "game"
    ...
    : flavor === "round"
      ? (isPast ? "Played Round" : "Upcoming Round")
      : (isPast ? "Past Trip" : "Golf Trip");
  ...
  return (
    <>
      ...
      <main style={{ minHeight: "100dvh", background: "#07100a", color: "#fff", paddingBottom: 100 }}>
          ...
          return (
            <div style={{ position: "relative", width: "100%", aspectRatio: cover ? "16/9" : undefined, minHeight: cover ? undefined : 220, overflow: "hidden", background: cover ? "#0d1f12" : "linear-gradient(135deg, #1c4425 0%, #07100a 100%)" }}>
              ...
              <div style={{ position: "absolute", left: 18, right: 18, bottom: 18, display: "flex", flexDirection: "column", gap: 8 }}>
                <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(255,255,255,0.7)" }}>{flavorEyebrow}</div>
                <div style={{ display: "flex", alignItems: "flex-end", gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {roundCourse && (roundCourse.city || roundCourse.state) && (
                      <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 500, color: "rgba(255,255,255,0.82)", marginTop: 2, textShadow: "0 1px 6px rgba(0,0,0,0.6)" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1, minWidth: 0, paddingTop: 3 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 3 }}>
                  <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)" }}>{flavorEyebrow}</div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                      {/* Chat lives top-right with the trip-level actions
                          (not floating under the members) — visible to all
                          members, not just the owner. */}
                      {!isRound && (
                        <button
                ...
                {(trip.startDate || trip.endDate) && (
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 6 }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#4da862" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                    <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, fontWeight: 600, letterSpacing: "0.01em", color: "rgba(255,255,255,0.92)" }}>
                      {isRound
                ...
                {!isRound && (trip.arrivalAirport || trip.lodging || (trip.cities?.length ?? 0) > 0) && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
                    ...
                    {trip.lodging && (
                      <button
                        onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([trip.lodging, trip.lodgingCity, trip.lodgingState].filter(Boolean).join(", "))}`, "_blank")}
                        aria-label={`Open ${trip.lodging} in Google Maps`}
                        style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 9px", borderRadius: 99, background: "rgba(77,168,98,0.1)", border: "1px solid rgba(77,168,98,0.28)", fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 600, color: "rgba(126,200,140,0.95)", maxWidth: 220, cursor: "pointer" }}
                      >
                        {/* Clean single-bed glyph — legible at 11px where the
                            old multi-line hotel icon read as a blob. */}
                        ...
                        <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{trip.lodging}</span>
                        {/* External-link hint so it reads as tappable */}
                        <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: 0.7 }}>
                  ...
                  return (
                    <div key={dayKey} style={{ marginBottom: 18 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                        <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 800, letterSpacing: "0.16em", color: "#4da862" }}>{dayHeader(dayKey)}</div>
                          ...
                          <div key={tc.id} style={{ position: "relative", overflow: "hidden", borderRadius: 12, marginBottom: 6 }}>
                            ...
                            >
                              ...
                              <div onClick={e => { if (swipedId === tc.id) { e.stopPropagation(); setSwipedId(null); return; } router.push(`/courses/${tc.course.id}`); }} style={{ flex: 1, minWidth: 0, cursor: "pointer", alignSelf: "center" }}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3, fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 500, color: "rgba(255,255,255,0.62)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        ...
        {/* Scorecard photos — keepsake pictures of the paper card, attached
            to the round/trip itself (not a game) so they're always reachable,
            even after the date has passed. Anyone can view; members/creator
            can add or remove. */}
        {(() => {
          const photos = trip.scorecardPhotos ?? [];
            ...
            <div style={{ padding: "24px 20px 0" }}>
              ...
              {photos.length > 0 && (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: canEdit ? 12 : 0 }}>
                  {photos.map((p) => (
                    <div key={p} style={{ position: "relative", aspectRatio: "1", borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)" }}>
                      <img
                        src={cdnImage(p)}
                        alt="Scorecard"
                      ...
                      {canEdit && (
                        <button
                          onClick={() => removeScorecardPhoto(p)}
                          aria-label="Remove photo"
                          style={{ position: "absolute", top: 5, right: 5, width: 22, height: 22, borderRadius: "50%", background: "rgba(0,0,0,0.6)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                        >
          ...
          <div style={{ background: "#0d1f14", borderRadius: "20px 20px 0 0", border: "1px solid rgba(255,255,255,0.08)", display: "flex", flexDirection: "column", maxHeight: "88vh" }}>
            <div style={{ padding: "14px 20px 12px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 500, color: "rgba(255,255,255,0.62)", marginTop: 1 }}>{viewGame.courseName} · {viewGame.players?.length || 0} players</div>
              </div>
              <button
      ...
      <input
        ...
        onChange={(e) => {
          const file = e.target.files?.[0];
          e.target.value = "";
          if (file) uploadScorecardPhoto(file);
        }}
      />
