...
function CropModal({ file, aspect, label, onDone, onClose }: {
...
export default function AdminCourseEditPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const { data: row } = await supabase
        .from("Course")
        .select(`id, name, city, state, country, latitude, longitude, websiteUrl, phone, description, holeCount, isVerified, isPublic, coverImageUrl, logoUrl,
  ...
  const uploadCroppedImage = async (blob: Blob, type: "cover" | "logo") => {
    setImageUploading(true);
    const supabase = createClient();
    const storagePath = `course-images/${courseId}-${type}.jpg`;
    await supabase.storage.from("tour-it-photos").upload(storagePath, blob, { contentType: "image/jpeg", upsert: true });
    const { data: { publicUrl } } = supabase.storage.from("tour-it-photos").getPublicUrl(storagePath);
    await supabase.from("Course").update({ [type === "cover" ? "coverImageUrl" : "logoUrl"]: publicUrl, updatedAt: new Date().toISOString() }).eq("id", courseId);
    if (type === "cover") setCoverUrl(publicUrl); else setLogoUrl(publicUrl);
    setCropTarget(null); setCropFile(null); setImageUploading(false);
  };
  ...
  return (
    <div style={{ minHeight: "100svh", background: "#07100a", color: "#fff", fontFamily: "'Outfit', sans-serif", paddingBottom: 60 }}>
      ...
      <div style={{ padding: "20px 16px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {/* ── Photos ── */}
        <section style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 700, marginBottom: 2 }}>Photos</div>

          {/* Cover photo */}
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Cover Photo (16:9)</div>
            <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", borderRadius: 10, overflow: "hidden", background: "rgba(255,255,255,0.04)", border: "1px dashed rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {coverUrl
                ? <img src={coverUrl} alt="cover" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : <span style={{ fontSize: 12, color: "rgba(255,255,255,0.3)" }}>No cover photo</span>
              }
              <button onClick={() => coverInputRef.current?.click()} disabled={imageUploading} style={{ position: "absolute", bottom: 8, right: 8, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(6px)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "6px 12px", fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 600, color: "#fff", cursor: "pointer" }}>
                {imageUploading && cropTarget === "cover" ? "Uploading…" : coverUrl ? "Replace" : "Upload"}
              </button>
            </div>
            <input ref={coverInputRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (f) { setCropFile(f); setCropTarget("cover"); } e.target.value = ""; }} />
          </div>

          {/* Logo */}
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Logo (1:1)</div>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <button onClick={() => logoInputRef.current?.click()} disabled={imageUploading} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, padding: "8px 16px", fontFamily: "'Outfit', sans-serif", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer" }}>
                {imageUploading && cropTarget === "logo" ? "Uploading…" : logoUrl ? "Replace logo" : "Upload logo"}
              </button>
            </div>
            <input ref={logoInputRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (f) { setCropFile(f); setCropTarget("logo"); } e.target.value = ""; }} />
          </div>
        </section>
      ...
      {cropFile && cropTarget && (
        <CropModal
          ...
          onDone={blob => uploadCroppedImage(blob, cropTarget)}
